...
#define FACTORY_RESET_PRESS_TIMEOUT_MS 8000  // 12 seconds pressed.
...
// check JLED Later.
JLed ledController = JLed(PIN_STATUS_LED).LowActive();
...
void checkPressCountTimeout() {
  ...
  if (!isValidTime) {
    ...
    ledController.FadeOff(180).Repeat(3);
...
void sysEnabledBlink() {
  if (systemEnabled) {
    ledController.Blink(100, 100).Repeat(2).DelayAfter(2000).Forever();
  }
}
